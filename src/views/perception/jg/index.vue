<template>
  <div class="app-container">
    <el-row :gutter="20">
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="deviceList" max-height="650">
          ...
          <el-table-column label="闭合状态" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.status === 1 ? "开启" : "闭合" }}</span>
            </template>
          </el-table-column>
          <el-table-column label="告警规则" align="center">
            <template slot-scope="scope">
              <el-switch
                v-model="scope.row.openAlert"
                :active-value="1"
                :inactive-value="0"
                @change="handleAlertStatus(scope.row)"
              ></el-switch>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-button
                ...
                @click="onSetHeartBeat(scope.row)"
                >设置心跳</el-button
              >
              <el-button
                size="mini"
              ...
              <el-button
                ...
                type="text"
                icon="el-icon-document"
                @click="handleHistoryAlert(scope.row)"
                >历史告警</el-button
              >
              <el-button
                size="mini"
          ...
          <el-col v-if="form.cameraPreset">
            <el-form-item label="预置图片：">
              <el-image
                ...
                :src="viewFileNoAuth(presetImage)"
                :preview-src-list="[viewFileNoAuth(presetImage)]"
              ></el-image>
            </el-form-item>
          </el-col>
          <el-col>
    ...
    <el-dialog
      title="设置心跳"
      :visible.sync="openHeartBeat"
      width="500px"
      append-to-body
      :before-close="closeHeartBeat"
    >
      <el-form ref="form1" :model="form1" :rules="rules1" label-width="120px">
        <el-col>
          <el-form-item label="推送间隔时间：" prop="interval">
            <el-input
              type="number"
              v-model="form1.interval"
              placeholder="请输入推送间隔时间"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="closeHeartBeat">取 消</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      title="历史告警"
      :visible.sync="openHistoryAlert"
      width="50%"
      append-to-body
      :before-close="closeHistoryAlert"
    >
      ...
      <pagination
        ...
        @pagination="getHistoryAlertList"
      />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onSetHeartBeat(row) {
      ...
      this.form1.interval = row.heartbeatInterval;
    ...
    submitForm1: function () {
      this.$refs["form1"].validate((valid) => {
        if (valid) {
          setHeartBeat(this.deviceId, this.form1).then((response) => {
    ...
    closeVideo() {
      ...
      this.$refs.preview3.stopVideoPlay();
    },
  },
};
</script>
